  ...
  create or replace view `stockmarket-455214`.`dbt_analytics_intermediate`.`int_daily_aggregates`
  OPTIONS()
  as 
...
WITH daily_batch_data AS (
    SELECT
        ...
        daily_change_pct
    FROM
        `stockmarket-455214`.`dbt_analytics_staging`.`stg_daily_prices`
),
...
daily_streaming_data AS (
    ...
    FROM
        `stockmarket-455214`.`dbt_analytics_staging`.`stg_realtime_prices`
    GROUP BY
        symbol, date
)
