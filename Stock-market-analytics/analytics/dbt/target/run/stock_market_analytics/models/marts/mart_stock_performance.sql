    ...
    create or replace table `stockmarket-455214`.`dbt_analytics_marts`.`mart_stock_performance`
    partition by date
...
WITH technical_data AS (
    SELECT
        ...
        volatility_20d
    FROM
        `stockmarket-455214`.`dbt_analytics_intermediate`.`int_technical_indicators`
),
